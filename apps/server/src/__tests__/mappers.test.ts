...
describe('pageTitle', () => {
  it('prefers Name, then Task, then Title', () => {
    expect(pageTitle({ properties: { Name: title('N'), Task: title('T'), Title: title('Ti') } })).toBe('N')
...
describe('pageToTask', () => {
  it('maps id, name, due and status', () => {
    expect(
      pageToTask({
        id: 't1',
        properties: { Name: title('Task 1'), Due: { date: { start: '2026-07-01' } }, Status: { status: { name: 'Todo' } } },
      })
...
describe('pageToNote', () => {
  it('resolves an emoji icon and last edited time', () => {
    expect(
      pageToNote({ id: 'n1', icon: { emoji: '📝' }, last_edited_time: '2026-01-01', properties: { Name: title('Note') } })
    ).toEqual({ id: 'n1', name: 'Note', icon: '📝', lastEdited: '2026-01-01' })
  })

  it('falls back through external then file icon urls', () => {
    expect(pageToNote({ id: 'n2', icon: { external: { url: 'http://x/e.png' } }, properties: { Name: title('E') } }).icon).toBe(
      'http://x/e.png'
    )
    expect(pageToNote({ id: 'n3', icon: { file: { url: 'http://x/f.png' } }, properties: { Name: title('F') } }).icon).toBe(
      'http://x/f.png'
    )
  })
...
describe('pageToProject', () => {
  it('maps id, name and status', () => {
    expect(pageToProject({ id: 'p1', properties: { Name: title('Proj'), Status: { status: { name: 'Doing' } } } })).toEqual({
      id: 'p1',
      name: 'Proj',
      status: 'Doing',
    })
  ...
  it('leaves status undefined when absent', () => {
    expect(pageToProject({ id: 'p2', properties: { Name: title('Proj2') } }).status).toBeUndefined()
  })
})

describe('pageToTag', () => {
  it('maps id and name only', () => {
    expect(pageToTag({ id: 'g1', properties: { Name: title('Tag') } })).toEqual({ id: 'g1', name: 'Tag' })
  })
})
