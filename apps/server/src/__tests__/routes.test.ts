...
const db: TenantDb = { tasks: 'tasks-db', notes: 'notes-db', projects: 'projects-db', tags: 'tags-db' }
...
function ctx(notion: ReturnType<typeof fakeNotion>, overrides: Partial<RouteContext> = {}): RouteContext {
  return { params: {}, body: undefined, notion: notion as any, db, ...overrides }
}

const titlePage = (id: string, name: string, extra: Record<string, any> = {}) => ({
  id,
...
describe('view routes', () => {
  it('queries the tenant database and maps results', async () => {
    ...
    expect(res).toEqual({ status: 200, body: { tasks: [{ id: 't1', name: 'Task 1', dueDate: undefined, status: 'Todo' }] } })
  })
...
describe('POST /api/tasks (create)', () => {
  ...
  it('creates a task in the tenant tasks db', async () => {
    ...
    const res = await route('POST', '/api/tasks').handler(ctx(notion, { body: { name: 'Buy milk' } }))
...
describe('PATCH /api/tasks/:id/done', () => {
  it('marks the task Done', async () => {
    ...
    const res = await route('PATCH', '/api/tasks/:id/done').handler(ctx(notion, { params: { id: 't1' } }))
...
describe('DELETE /api/tasks/:id', () => {
  it('moves the task to the trash', async () => {
    ...
    const res = await route('DELETE', '/api/tasks/:id').handler(ctx(notion, { params: { id: 't1' } }))
...
describe('GET /api/tasks/:id/metadata', () => {
  it('resolves the project relation name and due date', async () => {
    ...
    notion.pages.retrieve
      .mockResolvedValueOnce({ properties: { Due: { date: { start: '2026-07-01' } }, Project: { relation: [{ id: 'p1' }] } } })
      .mockResolvedValueOnce(titlePage('p1', 'Website'))

    const res = await route('GET', '/api/tasks/:id/metadata').handler(ctx(notion, { params: { id: 't1' } }))
  ...
  it('returns null project when there is no relation', async () => {
    const notion = fakeNotion()
    notion.pages.retrieve.mockResolvedValueOnce({ properties: { Due: { date: { start: '2026-07-01' } }, Project: { relation: [] } } })
...
describe('POST /api/logs (public)', () => {
  ...
  it('accepts a valid log line', async () => {
    const res = await route('POST', '/api/logs').handler({ params: {}, body: { level: 'warn', line: 'hi' } })
