...
function tryMatch(route: Route, method: string, pathSegments: string[]): Record<string, string> | undefined {
  ...
  const matched = patternSegments.every((pattern, i) =>
    pattern.startsWith(':') ? ((params[pattern.slice(1)] = pathSegments[i]), true) : pattern === pathSegments[i]
  )
